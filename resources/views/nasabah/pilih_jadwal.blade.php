...
<body>
            ...
            <div class="container row mt-4">
                ...
                <div class="btn-sign col-2 justify-content-center align-items-start d-flex gap-3 ">
                    <div class="d-flex justify-content-center align-items-center  gap-3">
                        ...
                        <a href="/nasabah/profile" style="color: black" class=""><b id="nama_profil1"></b></a>
                    </div>
        ...
        <div class="formulir bawah container-fluid row d-flex justify-content-center align-items-center gap-3">
            <div class="col-6 formulir-kiri d-flex justify-content-around align-items-center">
                <div class="formulir-dalam">
                    <form>
                        <h3 class="">Pilih Jadwal yang Tersedia</h3>

                        <div class="mb-3">
                            <label for="exampleInputtanggal1" class="form-label">Tanggal Jemput</label>
                            <input type="date" class="form-control" id="tanggal" aria-describedby="tanggal">
                        </div>

                        <div class="mb-3">
                            <label for="Select" class="form-label">Jam Jemput</label>
            ...
            <div class="col-4 formulir-kanan ">
                <h3 class="px-5 py-4">Ringkasan Penjemputan</h3>
                ...
                <div class="harga-sampah d-flex justify-content-between px-5">
                    <p><span>Tanggal Hari Jemputan</span></p>
                    <p id="tanggalFix"></p>
                </div>

                <div class="harga-sampah d-flex justify-content-between px-5">
                    <p><span>Jam Jemputan</span></p>
                    <p id="jadwal"></p>
                </div>

                <a><button type="button" class="button-green mx-5" id="submit">Minta
                        Jemput</button></a>
            </div>
        </div>
        <!-- FORMULIR-END -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            axios.get('/api/jenis/' + localStorage.getItem('jenis_sampah'), {
                headers: {
                    'Authorization': 'bearer ' + $token
                }
            }).then(function (response) {
            ...
            $('#total_Berat').text(localStorage.getItem('berat'))
            $('#alamat').text(localStorage.getItem('alamat_jemput'))

            $('#tanggal').change(function () {
                localStorage.setItem('tanggal', $('#tanggal').val())
                $('#tanggalFix').text($('#tanggal').val())
            })

            $('select').change(function () {
                localStorage.setItem('jadwal', $('select').val())
                $('#jadwal').text($('select').val())
            })
            ...
            $('#submit').click(function () {
                axios.get('/api/nasabah/' + localStorage.getItem('id'), {
                    ...
                    console.log(response)
                    axios.post('/api/jemput', {
                        id_nasabah: response.data.id_nasabah,
                        tanggal: localStorage.getItem('tanggal'),
                        alamat: localStorage.getItem('alamat_jemput'),
                        jadwal: localStorage.getItem('jadwal'),
                        jenis_sampah: localStorage.getItem('jenis_sampah'),
                        berat: localStorage.getItem('berat'),
                        harga: localStorage.getItem('totalHarga')
                    }, {
                        headers: {
                            'Authorization': 'bearer ' + $token
                        }
                        ...
                        if (response.data.success) {
                            window.location = '/nasabah/jemput/finalisasi'
                        }
                    })
                })
            })
